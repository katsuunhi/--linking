	...
	for (var i = 0; i < gameXSize / 2; i++) {
		...
		for (var m = 0; m < gameYSize; m++) {
			if (i == 0 || m == 0 || m == gameYSize - 1) {
				...
				// var temp1 = Math.floor(Math.random() * imageKinds) + 1;
				// var temp2 = Math.floor(Math.random() * imageKinds) + 1;
				var cls = Math.floor(Math.random()*4);
				var temp1 = Math.ceil(Math.random() * 10) + cls*10;
				var temp2 = Math.ceil(Math.random() * 10) + cls*10;
...
function drawMap() {
	...
	for (x in mapArray) {
		...
		for (y in mapArray[x]) {
			...
			if (mapArray[x][y] != 0) {
				...
				// var imgIdx = Math.ceil(Math.random()*10);

				cxt.drawImage(drawingImg, x_position, y_position, 45, 45);
	...
	if (cellOneOn) {
		...
		cxt.strokeRect(cellOnePos.xPos, cellOnePos.yPos, 45, 45)
	}
	if (isRemoveLineOn) {
		cxt.lineWidth = 4;
		cxt.strokeStyle = "rgba(255,100, 0, 0.6)";
		cxt.strokeRect(cellTwoPos.xPos, cellTwoPos.yPos, 45, 45)
...
function drawCell(row, column) {
	...
	if (cellOneOn) {
		...
		if (deal()) {
			var a = Math.ceil(mapArray[cellOnePos.xIndex][cellOnePos.yIndex]/10);
			var b = Math.ceil(mapArray[cellTwoPos.xIndex][cellTwoPos.yIndex]/10);
			if (a != b) {
